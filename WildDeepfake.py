...
def f1(path, mode, label):
    ...
    squens=[os.path.join(video,squen) for video in videos for squen in os.listdir(os.path.join(path,video))]
    print(f'squens: {len(squens)}')
    infos = [os.path.join(squen,file)+'\t'+bin_label+'\n' for squen in squens for file in os.listdir(os.path.join(path,squen))]
