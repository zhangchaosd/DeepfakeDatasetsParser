...
def crop_save(img, crop_data, path, faces_prefix, rela_path, image_name):
    ...
    return os.path.join(faces_prefix, rela_path, face_name)

def parse_image(info, mode, image_folder, masks_folder, path, faces_prefix, face_scale, detector):
...
def f1(mode, image_folder, masks_folder, path, faces_prefix, face_scale, detector, num_workers):
...
def main(path, face_scale, detector, num_workers):
    ...
    for mode in modes:
        all_infos += f1(mode[0], mode[1], mode[2], path, faces_prefix, face_scale, detector, num_workers)
...
if __name__ == '__main__':
    args = parse()
    main(args.path, args.scale, args.detector, args.workers)